...
class wsdl extends nusoap_base {
			...
			switch ($this->status) {
				...
				case 'portType':
					...
					switch ($name) {
						...
						default:
							if (!empty($this->currentPortOperation))
							{
	 ...
	 * contrary to the method name, this is not limited to RPC
	 */
	function createExampleRequest( $operation, $direction, $bindingType = 'soap' )
	{
		if ($direction != 'input' && $direction != 'output')
		{
			return false;
		...
		if (isset($opData[$direction]['parts']) && sizeof($opData[$direction]['parts']) > 0)
		{
			...
			foreach( $parts as $name => $type )
			{
				...
				$xml .= $this->constructExampleXml($name, $type, null, $use, $enc_style);
			}
		}
	...
	function constructExampleXml( $name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false )
	{
		...
		if( strpos($type, ':') )
		{
			...
			if( $ns == $this->XMLSchemaVersion || $ns == 'http://schemas.xmlsoap.org/soap/encoding/' )
			{
				...
				if( is_null( $value ) )
				{
					$xml = "<$name$elementNS>[" . $uqType . "]</$name>";
					/*if ($use == 'literal') {
		...
		if ($phpType == 'struct')
		{
			...
			$elementAttrs = $this->serializeComplexTypeAttributes($typeDef, array( 1, 2 ), $ns, $uqType);
			$xml .= "<$elementName$elementNS$elementAttrs>";

			$xml .= $this->serializeComplexTypeElements_example( $typeDef, $value, $ns, $uqType, $use, $encodingStyle );
	...
	function serializeComplexTypeElements_example( $typeDef, $value, $ns, $uqType, $use = 'encoded', $encodingStyle = false )
	{
		$xml = '';
		...
		if( isset( $typeDef['elements'] ) && is_array( $typeDef['elements'] ) )
		{
			...
			foreach( $typeDef['elements'] as $eName => $attrs )
			{
				...
				$xml .= $this->constructExampleXml( $eName, isset($attrs['type']) ? $attrs['type'] : $attrs['ref'], $v, $use, $encodingStyle, $unqualified );
			}
		}
